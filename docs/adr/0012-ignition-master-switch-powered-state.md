...
- **Charge-health warning** — while RUNNING, a marginal-charging warning ("loads too high or
  R/R weak") with a latched `chargeMarginal()` flag for a future tablet alert / load-shed. To
  avoid false triggers from bus ripple and brief load-switch dips near the threshold, it uses
  **hysteresis + debounce**: go marginal when `Vbus < CHARGE_WARN` (13.4 V) sustained 5 s; clear
  when `Vbus >= CHARGE_CLEAR` (13.6 V) sustained 5 s; the 13.4–13.6 deadband holds state between.
  The INA3221 driver will additionally hardware-average samples to pre-filter noise. Clean ladder
  (no overlap): `V_RUN_OFF 12.9 < V_RUN_ON 13.2 < CHARGE_WARN 13.4 < CHARGE_CLEAR 13.6`.
